...
proc addBrandingHeaders(ctx: Context): Future[void] {.async, gcsafe.} =
  ...
  discard ctx.setHeader("X-Rakta-Version", "0.2.1")
  await ctx.next()
...
proc all*(app: App, pattern: string, handlers: varargs[Handler]) =
  ## Registers a route handler for ALL HTTP methods on the specified pattern.
  ## 
  ## This method will register the same handler for GET, POST, PUT, DELETE, PATCH, and HEAD methods.
  ## Useful for creating catch-all routes or when you want the same logic to handle all HTTP methods.
  ## 
  ## Parameters:
  ##   pattern: URL pattern to match (e.g., "/api/*", "/admin/:section")
  ##   handlers: Variable number of handlers where the last one is the route handler
  ##            and preceding ones are middlewares
  ## 
  ## Example:
  ##   ```nim
  ##   # Handle all methods for a catch-all route
  ##   app.all("/api/*", proc(ctx: Context): Future[void] {.async.} =
  ##     await ctx.json(%*{
  ##       "method": $ctx.req.httpMethod,
  ##       "path": ctx.req.path,
  ##       "message": "This endpoint handles all HTTP methods"
  ##     })
  ##   )
  ##   
  ##   # With middlewares
  ##   app.all("/admin/*", authMiddleware, loggingMiddleware, proc(ctx: Context): Future[void] {.async.} =
  ##     await ctx.send("Admin area - method: " & $ctx.req.httpMethod)
  ##   )
  ##   ```
  if handlers.len == 0:
    return
...
proc all*(app: App, pattern: string, handler: Handler) =
  ## Registers a route handler for ALL HTTP methods on the specified pattern.
  ## 
  ## This method will register the same handler for GET, POST, PUT, DELETE, PATCH, and HEAD methods.
  ## Useful for creating catch-all routes or when you want the same logic to handle all HTTP methods.
